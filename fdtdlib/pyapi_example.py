...
def sweep_script(ring_index=2.):
    ...
    fdtd.addgaussian(x=0., y=-0.4e-6, injection_axis="y", waist_radius_w0=0.2e-6,
                     wavelength_start=0.5e-6, wavelength_stop=0.6e-6)
    fdtd.addring(x=0.0e-9, y=0.0e-9, z=0.0e-9, inner_radius=0.1e-6,
                 outer_radius=0.2e-6, index=float(ring_index))
    fdtd.addmesh(dx=10.0e-9, dy=10.0e-9, x=0.,
                 y=0., x_span=0.4e-6, y_span=0.4e-6)
    ...
    props = OrderedDict([("name", "power"), ("override global monitor settings", True), (
        "x", 0.), ("y", 0.4e-6), ("monitor type", "linear x"), ("frequency points", 10.0)])
...
for x in numpy.arange(2, 4, 1):
    ...
    if x == 3:
        ...
        # data on uniform grid, selecting the x-component of first frequency
        Ex = E_image["E"][:, :, 0, 0, 0]
        Ex_abs = abs(Ex)
        ...
        # finding the location of max abs(Ex)
        indices = numpy.where(Exi_abs == Exi_abs.max())
