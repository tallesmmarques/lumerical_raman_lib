...
class Fdtd(object):
    def __init__(self, name="fdtd_file.fsp"):
        ...
        self.fdtd.importmaterialdb(
            "D:\\TallesArquivos\\Documents\\FotonicaArquivos\\raman\\fdtdlib\\material.mdf")
    ...
    def get_type(self, simbol: str, rindex: int, cindex: int):
        if simbol == '0':
            ...
            return {'type': 'junction', 'radius': 0.38}
            ...
            for cindex, simbol in enumerate(row):
                ...
                if type['type'] == 'none':
                    ...
                    vtx = [
                        (0, r),
                        (a, r),
                        (2.5*a + np.cos(np.deg2rad(120)) *
                            r, h + np.sin(np.deg2rad(120))*r),
                    ...
                    props = OrderedDict([("name", f'source_{self.source_index}'),
                                         ...
                                         ('pulse type', 'standard'),
                                         ('frequency', (c/1541.3e-9)),  # Hz
                                         ('offset', 30e-15),  # fs
                                         ('pulselength', 50e-15),  # fs
